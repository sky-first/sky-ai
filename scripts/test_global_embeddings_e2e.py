...
async def test_end_to_end_global_embeddings():
    ...
    async with async_session() as db:
        ...
        try:
            ...
            await db.execute(
                text(
                    "INSERT INTO spaces (id, name, created_at) VALUES (:id, 'Test Space A', NOW())"
                ),
                {"id": space_id_A},
            )
        ...
        try:
            ...
            inserted = await run_metadata_ingestion(
                db=db, connection_id=conn_id, space_id=None  # GLOBAL
            )
        ...
        res = await db.execute(
            select(TableMetadata).where(TableMetadata.data_connection_id == conn_id)
        )
        ...
        # Check Embeddings
        res = await db.execute(
            select(EmbeddingRecord).where(EmbeddingRecord.table_metadata_id == meta.id)
        )
        ...
        if len(results_invalid) == 0:
            ...
            print(
                f"❌ FAILED! Leaked {len(results_invalid)} records from wrong connection!"
            )
...
async def cleanup(db, conn_id, space_id):
    ...
    try:
        ...
        await db.execute(
            text("DELETE FROM connection_metadata WHERE connection_id = :id"),
            {"id": conn_id},
        )

        # Fetch metadata IDs first
        res = await db.execute(
            select(TableMetadata.id).where(TableMetadata.data_connection_id == conn_id)
        )
        ...
        if meta_ids:
            await db.execute(
                delete(EmbeddingRecord).where(
                    EmbeddingRecord.table_metadata_id.in_(meta_ids)
                )
            )
            await db.execute(
                delete(TableMetadata).where(TableMetadata.id.in_(meta_ids))
            )

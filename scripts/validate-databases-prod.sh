...
echo "1️⃣  PostgreSQL StatefulSet..."
echo ""
kubectl get statefulset -n $NAMESPACE postgresql -o wide || {
    echo "❌ PostgreSQL StatefulSet não encontrado"
    exit 1
}
...
kubectl logs -n $NAMESPACE $POD | tail -10 || echo "⚠️  Sem logs disponíveis"

# 2. Testar PostgreSQL connection
echo ""
echo "2️⃣  Testando PostgreSQL connectivity..."
echo ""
...
# 3. Verificar Redis
echo ""
echo "3️⃣  Redis StatefulSet..."
echo ""
kubectl get statefulset -n $NAMESPACE redis -o wide || {
    echo "❌ Redis StatefulSet não encontrado"
}
...
if [ -n "$REDIS_POD" ]; then
    kubectl logs -n $NAMESPACE $REDIS_POD 2>/dev/null | tail -10 || echo "⚠️  Sem logs disponíveis"
else
    echo "⚠️  Redis Pod não encontrado"
fi

# 4. Verificar PVCs
echo ""
echo "4️⃣  Persistent Volumes para Databases..."
echo ""
...
echo ""
echo "========================================="
echo "✅ Validação de Databases Completa!"
echo "========================================="

...
echo -e "${BLUE}1️⃣ Verificando containers parados...${NC}"
...
echo -e "${BLUE}2️⃣ Localizando diretório do projeto...${NC}"
...
echo -e "${YELLOW}3️⃣ Iniciando todos os containers com docker compose...${NC}"
START_RESULT=$(run_vm_command "cd $PROJECT_DIR && docker compose up -d 2>&1")
echo "$START_RESULT"
echo ""

echo -e "${BLUE}4️⃣ Aguardando 15 segundos para containers iniciarem...${NC}"
sleep 15
echo ""

echo -e "${BLUE}5️⃣ Verificando status dos containers...${NC}"
STATUS=$(run_vm_command 'docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"')
echo "$STATUS"
echo ""

echo -e "${BLUE}6️⃣ Verificando containers críticos...${NC}"
echo ""
...
echo ""
echo -e "${BLUE}7️⃣ Verificando porta 80 (nginx)...${NC}"
PORT_80=$(run_vm_command 'docker ps --filter "name=ai_saas_proxy" --format "{{.Ports}}" | grep "0.0.0.0:80"')
if [ -n "$PORT_80" ]; then
    echo -e "${GREEN}✅ Porta 80 está mapeada: $PORT_80${NC}"
else
...
echo -e "${BLUE}8️⃣ Testando nginx localmente...${NC}"
...
if [ "$ALL_OK" = true ] && [ -n "$PORT_80" ]; then
    ...
    echo ""
    echo -e "${YELLOW}💡 Tente novamente:${NC}"
    echo "   cd $PROJECT_DIR && docker compose up -d"
    echo ""
    echo -e "${YELLOW}💡 Ou verifique logs:${NC}"

...
echo -e "${BLUE}1️⃣ Verificando se Docker está rodando...${NC}"
...
if echo "$DOCKER_STATUS" | grep -q "DOCKER_NAO_INSTALADO"; then
    echo -e "${RED}❌ Docker não está instalado na VM${NC}"
    exit 1
else
...
echo -e "${BLUE}2️⃣ Verificando containers rodando...${NC}"
...
if echo "$CONTAINERS_PS" | grep -q "ERRO_DIR"; then
    echo -e "${YELLOW}⚠️  Diretório do projeto não encontrado, tentando docker ps direto...${NC}"
...
if [ -z "$CONTAINERS_PS" ] || echo "$CONTAINERS_PS" | grep -qE "(Cannot connect|permission denied)"; then
    echo -e "${RED}❌ Erro ao verificar containers${NC}"
    echo "$CONTAINERS_PS"
else
...
echo -e "${BLUE}3️⃣ Verificando docker compose status...${NC}"
...
if echo "$COMPOSE_STATUS" | grep -q "ERRO_DIR"; then
    echo -e "${YELLOW}⚠️  Diretório do projeto não encontrado${NC}"
else
    ...
    if echo "$COMPOSE_STATUS" | grep -qE "(Exit|Dead|unhealthy|restarting)"; then
        echo ""
        echo -e "${RED}⚠️  ATENÇÃO: Alguns containers estão com problemas!${NC}"
        echo "$COMPOSE_STATUS" | grep -E "(Exit|Dead|unhealthy|restarting)" || true
    fi
fi
echo ""

echo -e "${BLUE}4️⃣ Verificando containers parados ou com erro...${NC}"
...
if [ -n "$STOPPED_CONTAINERS" ] && ! echo "$STOPPED_CONTAINERS" | grep -qE "(Cannot connect|permission denied|^$)"; then
    ...
    if [ "$STOPPED_COUNT" -gt 0 ]; then
        echo -e "${YELLOW}⚠️  Containers parados encontrados:${NC}"
        echo "$STOPPED_CONTAINERS" | grep -v "^$" | grep -v "\[stdout\]" | grep -v "\[stderr\]"
        echo ""
        echo -e "${BLUE}💡 Verificando logs dos containers parados...${NC}"
        ...
        echo -e "${GREEN}✅ Nenhum container parado encontrado${NC}"
    fi
else
    echo -e "${GREEN}✅ Nenhum container parado encontrado${NC}"
fi
...
echo -e "${BLUE}5️⃣ Verificando containers críticos...${NC}"
...
echo -e "${BLUE}6️⃣ Verificando saúde dos serviços...${NC}"
...
if echo "$PG_CHECK" | grep -q "accepting connections"; then
    ...
    echo -e "${RED}❌ Não está respondendo${NC}"
fi
...
echo -e "${BLUE}7️⃣ Verificando recursos do sistema...${NC}"
...
if [ -n "$MISSING_CONTAINERS" ]; then
    echo -e "${RED}❌ Containers faltando:${NC}$MISSING_CONTAINERS"
    echo ""
    echo -e "${YELLOW}💡 Para reiniciar os containers:${NC}"
    ...
    echo -e "${GREEN}✅ Todos os containers críticos estão rodando${NC}"
    echo ""

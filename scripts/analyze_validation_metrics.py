...
def print_analysis(stats: Dict):
    ...
    print(
        f"🚫 Perguntas bloqueadas: {stats['blocked_questions']} ({stats['blocked_questions']/stats['total_events']*100:.1f}%)"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analisa métricas de validação dos logs"
    )
    parser.add_argument(
        "log_file", nargs="?", help="Arquivo de log para analisar (opcional)"
    )

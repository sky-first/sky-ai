...
if [ -f .env ]; then
    ...
    echo -e "${RED}[ERROR] .env NÃO existe${NC}"
    ((ERRORS++))
fi
echo ""

# 2. Verificar variáveis críticas
echo -e "${BLUE}2.  Verificando variáveis críticas...${NC}"
MISSING_VARS=""
...
for var in POSTGRES_PASSWORD REDIS_PASSWORD JWT_SECRET_KEY ENCRYPTION_KEY; do
    if grep -q "^${var}=" .env 2>/dev/null; then
        ...
        if [[ "$VALUE" == *"secure_password"* ]] || [[ "$VALUE" == *"generate"* ]] || [[ "$VALUE" == *"here"* ]] || [ -z "$VALUE" ]; then
            echo -e "${YELLOW}[WARNING] ${var} tem valor placeholder ou vazio${NC}"
            MISSING_VARS="${MISSING_VARS} ${var}"
            ((WARNINGS++))
        else
            echo -e "${GREEN}[OK] ${var} está configurado${NC}"
        fi
    else
        echo -e "${RED}[ERROR] ${var} NÃO encontrado no .env${NC}"
        MISSING_VARS="${MISSING_VARS} ${var}"
        ((ERRORS++))
    fi
done
...
echo -e "${BLUE}3.  Verificando NEXT_PUBLIC_API_URL...${NC}"
if grep -q "^NEXT_PUBLIC_API_URL=" .env 2>/dev/null; then
    API_URL=$(grep "^NEXT_PUBLIC_API_URL=" .env | cut -d'=' -f2-)
    echo -e "${GREEN}[OK] NEXT_PUBLIC_API_URL=${API_URL}${NC}"
else
    echo -e "${YELLOW}[WARNING] NEXT_PUBLIC_API_URL não encontrado (usará default)${NC}"
    ((WARNINGS++))
fi
echo ""

# 4. Verificar CORS_ORIGINS
echo -e "${BLUE}4.  Verificando CORS_ORIGINS...${NC}"
if grep -q "^CORS_ORIGINS=" .env 2>/dev/null; then
    CORS=$(grep "^CORS_ORIGINS=" .env | cut -d'=' -f2-)
    if [[ "$CORS" == *"*"* ]] || [[ "$CORS" == *"0.0.0.0"* ]]; then
        echo -e "${RED}[ERROR] CORS_ORIGINS contém '*' ou '0.0.0.0' (vulnerabilidade de segurança)${NC}"
        ((ERRORS++))
    else
        echo -e "${GREEN}[OK] CORS_ORIGINS=${CORS}${NC}"
    fi
else
    echo -e "${YELLOW}[WARNING] CORS_ORIGINS não encontrado (usará default)${NC}"
    ((WARNINGS++))
fi
echo ""

# 5. Verificar containers
echo -e "${BLUE}5.  Verificando containers Docker...${NC}"
if command -v docker >/dev/null 2>&1; then
    if sudo docker compose ps 2>/dev/null | grep -q "Up"; then
        echo -e "${GREEN}[OK] Containers estão rodando${NC}"
        sudo docker compose ps 2>/dev/null | grep "Up" | head -5
    else
        echo -e "${YELLOW}[WARNING] Nenhum container rodando (normal se ainda não fez deploy)${NC}"
        ((WARNINGS++))
    fi
else
    echo -e "${YELLOW}[WARNING] Docker não encontrado${NC}"
    ((WARNINGS++))
fi
echo ""

# 6. Verificar permissões do .env
echo -e "${BLUE}6.  Verificando permissões do .env...${NC}"
if [ -f .env ]; then
    PERMS=$(stat -c '%a' .env 2>/dev/null || stat -f '%A' .env 2>/dev/null || echo "")
    if [ "$PERMS" = "600" ]; then
        echo -e "${GREEN}[OK] Permissões corretas (600)${NC}"
    else
        echo -e "${YELLOW}[WARNING] Permissões: $PERMS (recomendado: 600)${NC}"
...
if [ -d "../sky-poc-backend" ] && [ -d "../sky-poc-frontend" ] && [ -d "../sky-poc-ai" ]; then
    echo -e "${GREEN}[OK] Todos os repositórios encontrados${NC}"
else
    echo -e "${YELLOW}[WARNING] Alguns repositórios podem estar faltando${NC}"
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    echo -e "${GREEN}[OK] Tudo pronto para o próximo deploy!${NC}"
    exit 0
elif [ $ERRORS -eq 0 ]; then
    echo -e "${YELLOW}[WARNING] Pronto com $WARNINGS aviso(s)${NC}"
    echo "   Verifique os avisos acima, mas o deploy deve funcionar"
    exit 0
else
    echo -e "${RED}[ERROR] Encontrados $ERRORS erro(s) e $WARNINGS aviso(s)${NC}"
    echo ""

...
if [ ! -d "../sky-poc-backend" ]; then
    echo "⚠️  ../sky-poc-backend não encontrado"
    VALID=false
fi

if [ ! -d "../sky-poc-frontend" ]; then
    echo "⚠️  ../sky-poc-frontend não encontrado"
    VALID=false
fi

if [ ! -d "../sky-poc-ai" ]; then
    echo "⚠️  ../sky-poc-ai não encontrado"
    VALID=false
fi

if [ "$VALID" = "true" ]; then
    echo "✅ Todos os build contexts existem"
else
    echo "⚠️  Alguns build contexts não existem (pode ser normal se ainda não clonou)"
fi
...
if docker compose config > /dev/null 2>&1; then
    ...
    echo "❌ ERRO: docker-compose.yml inválido após correções"

...
echo "2. Verificando nginx/proxy..."
echo "----------------------------------------"
...
if [ -n "$PROXY_CONTAINER" ]; then
    ...
    echo "[ERROR] Container proxy/nginx não está rodando"
fi
echo ""
...
if [ -n "$PORT_80" ]; then
    ...
    echo "[ERROR] Porta 80 NÃO está escutando"
fi
echo ""
...
if [ -f "$NGINX_CONF" ]; then
    echo "[OK] Arquivo de configuração encontrado: $NGINX_CONF"
    ...
    if grep -q "return 301 https" "$NGINX_CONF" && ! grep -q "# return 301 https" "$NGINX_CONF"; then
        echo "[WARNING]PROBLEMA DETECTADO: nginx está redirecionando HTTP para HTTPS"
        
        # Verificar se certificados existem
        if [ -d "certs" ] && [ -f "certs/fullchain.pem" ] && [ -f "certs/privkey.pem" ]; then
            echo "[OK] Certificados SSL encontrados"
        else
            echo "[ERROR] Certificados SSL NÃO encontrados"
            ...
            if [ -f "docker/nginx/nginx.conf.http-only" ]; then
                ...
                echo "[OK] Configuração HTTP-only aplicada"
                ...
                echo "[ERROR] Arquivo nginx.conf.http-only não encontrado"
            fi
        fi
    else
        echo "[OK] Configuração parece correta"
    fi
else
    echo "[ERROR] Arquivo de configuração não encontrado: $NGINX_CONF"
fi
echo ""
...
if [ -f ".env" ]; then
    ...
    if grep -q "POSTGRES_PASSWORD" .env && grep -q "REDIS_PASSWORD" .env; then
        echo "[OK] Variáveis críticas configuradas"
    else
        echo "[WARNING]Algumas variáveis podem estar faltando"
    fi
else
    echo "[ERROR] Arquivo .env NÃO existe"
...
if [ -n "$PROXY_CONTAINER" ]; then
    ...
    sudo docker exec "$PROXY_CONTAINER" wget -q -O- http://backend:8000/health 2>&1 | head -3 || echo "[WARNING]Backend não acessível do proxy"
    echo ""
    echo "Testando se frontend está acessível do proxy:"
    sudo docker exec "$PROXY_CONTAINER" wget -q -O- http://frontend:3000 2>&1 | head -3 || echo "[WARNING]Frontend não acessível do proxy"
fi
echo ""
...
if [ -z "$PROXY_CONTAINER" ]; then
    echo "[ERROR] Container proxy não está rodando"
...
if [ $ISSUES -eq 0 ]; then
    ...
    echo "[WARNING]$ISSUES problema(s) encontrado(s)"
    echo ""

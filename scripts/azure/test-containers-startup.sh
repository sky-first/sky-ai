...
echo -e "${BLUE}1️⃣  Verificando Docker Compose...${NC}"
...
# 2. Verificar .env
echo -e "${BLUE}2️⃣  Verificando .env...${NC}"
if [ ! -f .env ]; then
...
echo -e "${BLUE}3️⃣  Verificando docker-compose.yml...${NC}"
...
echo -e "${BLUE}4️⃣  Validando docker-compose.yml...${NC}"
if ! sudo docker compose config >/dev/null 2>&1; then
    echo -e "${RED}❌ docker-compose.yml tem erros de sintaxe${NC}"
...
echo -e "${BLUE}5️⃣  Testando criação de containers (dry-run)...${NC}"
if ! sudo docker compose up --dry-run >/dev/null 2>&1; then
    echo -e "${YELLOW}⚠️  Dry-run não disponível (continuando...)${NC}"
else
...
echo -e "${BLUE}6️⃣  Verificando recursos disponíveis...${NC}"
...
if [ "$AVAILABLE_MEM" -lt 2048 ]; then
    echo -e "${YELLOW}⚠️  Pouca memória disponível (recomendado: 2GB+)${NC}"
else
...
echo -e "${BLUE}7️⃣  Verificando espaço em disco...${NC}"

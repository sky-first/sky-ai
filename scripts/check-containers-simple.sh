...
echo -e "${BLUE}1️⃣ Testando conectividade básica...${NC}"
if ping -c 1 -W 2 "$VM_IP" > /dev/null 2>&1; then
    echo -e "${GREEN}✅ VM está acessível (ping)${NC}"
else
    echo -e "${YELLOW}⚠️  Ping falhou (pode estar bloqueado, mas HTTP pode funcionar)${NC}"
fi
echo ""

echo -e "${BLUE}2️⃣ Testando endpoints HTTP...${NC}"
echo ""
...
echo -e "${BLUE}3️⃣ Verificando resposta HTTP detalhada...${NC}"
echo ""
...
if echo "$HTTP_HEADERS" | grep -q "HTTP/"; then
    ...
    echo -e "${RED}❌ Não foi possível obter headers${NC}"
    echo "$HTTP_HEADERS"
fi
echo ""
...
if [ "$STATUS_CODE" = "200" ] || [ "$STATUS_CODE" = "301" ] || [ "$STATUS_CODE" = "302" ]; then
    ...
    echo -e "${RED}❌ Timeout ou conexão recusada${NC}"
    FAILED=$((FAILED + 1))
else
...
echo -e "${BLUE}4️⃣ Diagnóstico...${NC}"
echo ""

if [ $FAILED -eq 0 ]; then
    echo -e "${GREEN}✅ Todos os endpoints estão respondendo${NC}"
    echo ""
    echo -e "${BLUE}💡 A aplicação parece estar funcionando!${NC}"
    ...
    echo ""
    echo -e "${BLUE}💡 Para verificar containers na VM:${NC}"
    echo "   bash scripts/check-containers-status.sh poc-sky poc-sky $VM_IP"
    echo ""
    echo -e "${BLUE}💡 Ou via Azure Portal:${NC}"

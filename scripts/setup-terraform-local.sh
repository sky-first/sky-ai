...
echo "1️⃣  Verificando Azure CLI..."
...
echo "2️⃣  Verificando autenticação Azure..."
if az account show >/dev/null 2>&1; then
    ...
    if az login --use-device-code >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}⚠️  Tentando login interativo...${NC}"
        az login || {
            echo -e "${RED}❌ Falha ao fazer login no Azure${NC}"
            exit 1
        }
    fi
...
echo "3️⃣  Verificando Terraform..."
...
echo -e "${GREEN}✅ Terraform encontrado: $TERRAFORM_VERSION${NC}"
echo ""

# 4. Navegar para diretório do Terraform
echo "4️⃣  Navegando para diretório do Terraform..."
cd "$TF_DIR" || {
    echo -e "${RED}❌ Diretório do Terraform não encontrado: $TF_DIR${NC}"
    exit 1
}
...
echo "5️⃣  Verificando workspace Terraform..."
...
echo "6️⃣  Verificando configuração do backend..."
if [ -f "backend.hcl" ]; then
    echo -e "${GREEN}✅ Arquivo backend.hcl encontrado${NC}"
    ...
    echo -e "${YELLOW}⚠️  Variáveis de backend encontradas, criando backend.hcl...${NC}"
    cat > backend.hcl << EOF
...
EOF
    BACKEND_CONFIG="-backend-config=backend.hcl"
    echo -e "${GREEN}✅ backend.hcl criado${NC}"
else
    echo -e "${YELLOW}⚠️  Backend remoto não configurado${NC}"
...
echo "7️⃣  Inicializando Terraform..."
if [ -n "$BACKEND_CONFIG" ]; then
    ...
    terraform init -input=false $BACKEND_CONFIG || {
        ...
        terraform init -input=false -reconfigure || {
            echo -e "${RED}❌ Falha ao inicializar Terraform mesmo sem backend${NC}"
            exit 1
        }
        echo -e "${YELLOW}⚠️  Terraform inicializado sem backend remoto${NC}"
    }
else
...
echo "8️⃣  Verificando acesso ao estado..."
if terraform output vm_public_ip >/dev/null 2>&1; then
    VM_IP=$(terraform output -raw vm_public_ip 2>/dev/null || echo "")
    if [ -n "$VM_IP" ]; then
        echo -e "${GREEN}✅ IP da VM obtido: $VM_IP${NC}"
    else
        echo -e "${YELLOW}⚠️  Estado não tem vm_public_ip ainda${NC}"
    fi
else
    echo -e "${YELLOW}⚠️  Não foi possível acessar outputs (pode ser normal se ainda não fez apply)${NC}"
fi
echo ""

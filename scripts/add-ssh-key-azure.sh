...
echo -e "${GREEN}✅ Azure CLI encontrado: $AZ_CLI${NC}"
echo ""
...
if [ ! -f "$SSH_PUB_KEY_FILE" ]; then
    echo -e "${RED}❌ Chave pública não encontrada: $SSH_PUB_KEY_FILE${NC}"
    exit 1
fi
...
if [ -z "$VM_INFO" ]; then
    echo -e "${YELLOW}⚠️  VM não encontrada pelo IP. Listando todas as VMs...${NC}"
    echo ""
    ...
    echo -e "${GREEN}✅ VM encontrada: $VM_NAME (RG: $VM_RG)${NC}"
fi
...
if $AZ_CLI vm user update \
    ...
    if ssh -i ~/.ssh/id_ed25519 -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new "$VM_USER@$VM_IP" "echo 'OK'" 2>&1 | grep -q "OK"; then
        ...
        echo -e "${YELLOW}⚠️  Chave adicionada, mas conexão ainda não funciona${NC}"

    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if echo "$result" | grep -q "Conflict"; then
            if [ $attempt -lt $max_attempts ]; then
                ...
                echo -e "${RED}❌ Timeout após ${max_attempts} tentativas${NC}"
                echo -e "${YELLOW}💡 O comando anterior pode estar travado${NC}"
                echo -e "${YELLOW}💡 Tente reiniciar a extensão de run-command na VM ou aguarde mais tempo${NC}"
                return 1
            fi
        else
            # Sucesso
...
echo -e "${BLUE}1️⃣ Verificando status atual dos containers...${NC}"
try_execute_with_retry 'docker ps -a --format "{{.Names}}: {{.Status}}" | head -10'
echo ""

echo -e "${YELLOW}2️⃣ Iniciando todos os containers...${NC}"
...
if echo "$START_RESULT" | grep -q "ERRO_AO_INICIAR"; then
    ...
    echo ""
    echo -e "${YELLOW}💡 Tentando iniciar um por vez...${NC}"
...
echo -e "${BLUE}3️⃣ Aguardando 20 segundos para containers iniciarem...${NC}"
sleep 20
echo ""

echo -e "${BLUE}4️⃣ Verificando status final...${NC}"
...
echo ""
echo -e "${BLUE}6️⃣ Verificando porta 80 (nginx)...${NC}"
PORT_80=$(try_execute_with_retry 'docker ps --filter "name=ai_saas_proxy" --format "{{.Ports}}" | grep "0.0.0.0:80"')
if [ -n "$PORT_80" ]; then
    echo -e "${GREEN}✅ Porta 80 está mapeada${NC}"
else
    echo -e "${RED}❌ Porta 80 NÃO está mapeada${NC}"
    ALL_OK=false
fi
echo ""

echo -e "${BLUE}7️⃣ Testando nginx localmente...${NC}"
...
if echo "$NGINX_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${GREEN}✅ Nginx está respondendo: $NGINX_TEST${NC}"
else
    echo -e "${YELLOW}⚠️  Nginx ainda não está respondendo: $NGINX_TEST${NC}"
...
if [ "$ALL_OK" = true ] && [ -n "$PORT_80" ]; then
    ...
    echo ""
    if echo "$NGINX_TEST" | grep -qE "200|301|302|404"; then
        echo -e "${GREEN}✅ Nginx está respondendo - o site deve estar acessível!${NC}"
    else
        echo -e "${YELLOW}⚠️  Aguarde mais 1-2 minutos para o nginx terminar de iniciar${NC}"
    fi
else
    ...
    echo ""
    echo -e "${YELLOW}💡 Ou tente reiniciar manualmente:${NC}"

...
echo -e "${MAGENTA}1. STATUS DO CONTAINER NGINX${NC}"
echo ""
NGINX_STATUS=$(run_vm_command 'docker ps --filter "name=ai_saas_proxy" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"')
if [ -n "$NGINX_STATUS" ]; then
    echo "$NGINX_STATUS"
    echo -e "${GREEN}[OK] Container nginx está rodando${NC}"
else
    echo -e "${RED}[ERROR] Container nginx NÃO está rodando!${NC}"
    echo ""
    echo -e "${YELLOW}[INFO] Tentando iniciar...${NC}"
    run_vm_command 'docker start ai_saas_proxy && sleep 5 && docker ps --filter "name=ai_saas_proxy" --format "{{.Names}}: {{.Status}}"'
fi
echo ""

echo -e "${MAGENTA}2. PORTAS EXPOSTAS DO CONTAINER${NC}"
echo ""
...
if [ -n "$PORTS" ]; then
    ...
    if echo "$PORTS" | grep -q "0.0.0.0:80"; then
        echo -e "${GREEN}[OK] Porta 80 está mapeada para 0.0.0.0 (acessível externamente)${NC}"
    else
        echo -e "${RED}[ERROR] Porta 80 NÃO está mapeada para 0.0.0.0${NC}"
    fi
    if echo "$PORTS" | grep -q "0.0.0.0:443"; then
        echo -e "${GREEN}[OK] Porta 443 está mapeada para 0.0.0.0${NC}"
    else
        echo -e "${YELLOW}[WARNING] Porta 443 não está mapeada (pode ser normal se não usar HTTPS)${NC}"
    fi
else
    echo -e "${RED}[ERROR] Nenhuma porta encontrada${NC}"
fi
echo ""

echo -e "${MAGENTA}3. PROCESSOS DENTRO DO CONTAINER NGINX${NC}"
echo ""
...
if [ -n "$NGINX_PROCESSES" ]; then
    ...
    if echo "$NGINX_PROCESSES" | grep -q "nginx"; then
        echo -e "${GREEN}[OK] Processo nginx está rodando${NC}"
    else
        echo -e "${RED}[ERROR] Processo nginx NÃO encontrado${NC}"
    fi
else
    echo -e "${RED}[ERROR] Não foi possível verificar processos${NC}"
fi
echo ""

echo -e "${MAGENTA}4. PORTAS ESCUTANDO DENTRO DO CONTAINER${NC}"
echo ""
...
if [ -n "$NGINX_LISTEN" ] && ! echo "$NGINX_LISTEN" | grep -q "COMANDO_NAO_DISPONIVEL"; then
    ...
    if echo "$NGINX_LISTEN" | grep -q ":80"; then
        echo -e "${GREEN}[OK] Nginx está escutando na porta 80${NC}"
    else
        echo -e "${RED}[ERROR] Nginx NÃO está escutando na porta 80${NC}"
    fi
else
    echo -e "${YELLOW}[WARNING] Comandos netstat/ss não disponíveis no container${NC}"
    echo -e "${BLUE}[INFO] Verificando de outra forma...${NC}"
    ...
    if echo "$NGINX_CURL" | grep -qE "200|301|302|404"; then
        echo -e "${GREEN}[OK] Nginx está respondendo (HTTP $NGINX_CURL)${NC}"
    else
        echo -e "${RED}[ERROR] Nginx não está respondendo: $NGINX_CURL${NC}"
    fi
fi
echo ""

echo -e "${MAGENTA}5. CONFIGURAÇÃO DO NGINX${NC}"
echo ""
NGINX_CONFIG=$(run_vm_command 'docker exec ai_saas_proxy nginx -t 2>&1')
if echo "$NGINX_CONFIG" | grep -q "successful"; then
    echo -e "${GREEN}[OK] Configuração do nginx está válida${NC}"
    echo "$NGINX_CONFIG" | grep -E "(test|successful|syntax)"
else
    echo -e "${RED}[ERROR] Configuração do nginx tem erros:${NC}"
    echo "$NGINX_CONFIG"
fi
echo ""

echo -e "${MAGENTA}6. TESTE DE CONECTIVIDADE INTERNA (dentro da VM)${NC}"
echo ""
...
if echo "$LOCAL_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${GREEN}[OK] $LOCAL_TEST${NC}"
else
    echo -e "${RED}[ERROR] $LOCAL_TEST${NC}"
fi
...
if echo "$LOCALHOST_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${GREEN}[OK] $LOCALHOST_TEST${NC}"
else
    echo -e "${RED}[ERROR] $LOCALHOST_TEST${NC}"
fi
...
if echo "$IP_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${GREEN}[OK] $IP_TEST${NC}"
else
    echo -e "${RED}[ERROR] $IP_TEST${NC}"
fi
echo ""

echo -e "${MAGENTA}7. VERIFICAÇÃO DE FIREWALL NA VM${NC}"
echo ""
...
if [ -n "$FIREWALL" ] && ! echo "$FIREWALL" | grep -q "IPTABLES_NAO_DISPONIVEL"; then
    ...
    echo -e "${YELLOW}[WARNING] Não foi possível verificar iptables${NC}"
    echo -e "${BLUE}[INFO] Verificando se há firewall ativo...${NC}"
    ...
    if [ -n "$UFW_STATUS" ] && ! echo "$UFW_STATUS" | grep -q "UFW_NAO_DISPONIVEL"; then
        ...
        echo -e "${YELLOW}[WARNING] UFW não disponível ou não configurado${NC}"
    fi
fi
echo ""

echo -e "${MAGENTA}8. LOGS DO NGINX (últimas 20 linhas)${NC}"
echo ""
NGINX_LOGS=$(run_vm_command 'docker logs ai_saas_proxy --tail 20 2>&1')
echo "$NGINX_LOGS" | tail -20
echo ""

echo -e "${MAGENTA}9. VERIFICAÇÃO DE PORTAS NA VM${NC}"
echo ""
...
if [ -n "$VM_PORTS" ] && ! echo "$VM_PORTS" | grep -q "COMANDO_NAO_DISPONIVEL"; then
    ...
    if echo "$VM_PORTS" | grep -q ":80"; then
        echo -e "${GREEN}[OK] Porta 80 está escutando na VM${NC}"
    else
        echo -e "${RED}[ERROR] Porta 80 NÃO está escutando na VM${NC}"
    fi
else
    echo -e "${YELLOW}[WARNING] Não foi possível verificar portas na VM${NC}"
fi
echo ""
...
if ! echo "$NGINX_STATUS" | grep -q "ai_saas_proxy"; then
    echo -e "${RED}[ERROR] PROBLEMA CRÍTICO: Container nginx não está rodando${NC}"
    ISSUES=$((ISSUES + 1))
fi

if ! echo "$PORTS" | grep -q "0.0.0.0:80"; then
    echo -e "${RED}[ERROR] PROBLEMA CRÍTICO: Porta 80 não está mapeada para 0.0.0.0${NC}"
    ISSUES=$((ISSUES + 1))
fi

if ! echo "$LOCAL_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${RED}[ERROR] PROBLEMA: Nginx não responde em localhost:80${NC}"
    ISSUES=$((ISSUES + 1))
fi

if ! echo "$IP_TEST" | grep -qE "200|301|302|404"; then
    echo -e "${YELLOW}[WARNING] PROBLEMA: Nginx não responde no IP externo${NC}"
    echo -e "${BLUE}[INFO] Isso pode ser:${NC}"
...
if [ $ISSUES -eq 0 ]; then
    echo -e "${GREEN}[OK] Tudo parece estar funcionando corretamente!${NC}"
    echo ""
    echo -e "${BLUE}[INFO] Se ainda houver ERR_CONNECTION_REFUSED:${NC}"
    ...
    echo ""
    echo -e "${YELLOW}[INFO] Soluções recomendadas:${NC}"
    if ! echo "$NGINX_STATUS" | grep -q "ai_saas_proxy"; then

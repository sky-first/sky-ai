...
def populate_glossary(space_id: str):
    ...
    common_terms = [
        (
            ...
            "subscription",
            "Sum of active subscription amounts per month",
        ),
        (
            "ARR",
            "Annual Recurring Revenue",
            "subscription",
            "MRR × 12, total yearly recurring revenue",
        ),
        (
            "Churn Rate",
            "Customer Churn",
        ...
        (
            ...
            "revenue",
            "Average revenue per customer multiplied by average customer lifetime",
        ),
        (
            "ARPU",
            "Average Revenue Per User",
            "revenue",
            "Total revenue divided by total number of users",
        ),
        (
            "NPS",
            "Net Promoter Score",
            "satisfaction",
            "Customer satisfaction metric from -100 to +100",
        ),
        (
            "ROI",
            "Return on Investment",
            "finance",
            "Net profit divided by investment cost, expressed as percentage",
        ),
        (
            "KPI",
            "Key Performance Indicator",
            "metrics",
            "Measurable value demonstrating effectiveness of objectives",
        ),
        (
            "SaaS",
            "Software as a Service",
            "business_model",
            "Cloud-based software delivery model with subscriptions",
        ),
        (
            "Active User",
            "Active User",
            "engagement",
            "User who performed key action in defined time period",
        ),
        (
            "Conversion Rate",
            "Conversion Rate",
        ...
        (
            ...
            "Gross Profit Margin",
            "finance",
            "(Revenue - COGS) / Revenue, expressed as percentage",
        ),
        (
            "Burn Rate",
            "Cash Burn Rate",
...
def populate_metrics(space_id: str):
    ...
    common_metrics = [
        (
            "MRR",
            "Monthly Recurring Revenue",
            "SUM(amount) WHERE subscription_status = 'active'",
            "Revenue",
        ),
        (
            "ARR",
            "Annual Recurring Revenue",
            "SUM(amount) * 12 WHERE subscription_status = 'active'",
            "Revenue",
        ),
        (
            "Total Customers",
        ...
        (
            ...
            "COUNT(DISTINCT customer_id) WHERE created_at >= DATE_TRUNC('month', CURRENT_DATE)",
            "Customers",
        ),
        (
            "Churn Rate",
            "Monthly Churn Rate",
            "(COUNT cancelled / COUNT total) * 100",
            "Retention",
        ),
        (
            "Average Order Value",
            "AOV",
            "SUM(amount) / COUNT(DISTINCT invoice_id)",
            "Revenue",
        ),
        (
            "Revenue Growth",
        ...
        (
            ...
            "COUNT(*) FROM subscriptions WHERE status = 'active'",
            "Subscriptions",
        ),
    ]
...
def validate_setup(space_id: str):
    ...
    try:
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text(
                    "SELECT COUNT(*) FROM business_glossary WHERE space_id = :space_id"
                ),
                {"space_id": space_id},
            )
            ...
            print(f"\n📊 Validation Results:")
            print(
                f"   Embeddings: {embeddings_count} {'✅' if embeddings_count > 0 else '❌'}"
            )
            print(
                f"   Glossary Terms: {glossary_count} {'✅' if glossary_count >= 10 else '⚠️'}"
            )
...
async def main():
    ...
    if success:
        ...
        print(f"\nNext Steps:")
        print(
            f"  1. Test RAG: Make a query and check logs for 'multi_layer_rag_retrieved'"
        )

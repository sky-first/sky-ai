...
from core.agents.generic_sql_agent import run_agent_once, AgentConfig, TableSchema, UserContext
...
def test_conversational_memory():
    ...
    sync_db_url = settings.database_url.replace("postgresql+asyncpg://", "postgresql://").replace("postgresql+psycopg2://", "postgresql://")
    engine = create_engine(sync_db_url)
    SyncSessionLocal = sessionmaker(bind=engine)
    data_source = SQLAlchemyDataSource(engine=engine, dialect=Dialect.POSTGRES, label="test_conn")
    ...
    if ("customer_id" in sql2 or "customer 5" in sql2 or "= 5" in sql2) and ("invoices" in sql2):

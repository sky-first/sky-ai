...
async def generate_embeddings(
    space_id: str = None, connection_id: str = None, crew_id: str = None
):
    ...
    async with SessionLocal() as db:
        try:
            ...
            if settings.use_local_models:
                print(
                    f"🤖 Usando Ollama Embedding Provider (Model: {settings.llm_model_embedding_local})"
                )
                embedding_provider = OllamaEmbeddingProvider(
                    model=settings.llm_model_embedding_local
                )
            else:
                print(
                    f"☁️  Usando OpenAI Embedding Provider (Model: {settings.embedding_model or 'text-embedding-3-large'})"
                )

        ...
        async with db.begin():
            ...
            for conn_id in conn_ids:
                ...
                stmt = select(TableMetadata).where(
                    TableMetadata.data_connection_id == conn_id
                )
                ...
                for key, records in grouped.items():
                    ...
                    if duplicates:
                        ...
                        print(
                            f"  Merging {len(duplicates)} duplicates for {key} into {master.id}"
                        )
                        ...
                        await db.execute(
                            ...
                            .values(
                                table_metadata_id=master.id, space_id=None, crew_id=None
                            )
                        )

                        # 2. Delete duplicate metadata rows
                        await db.execute(
                            delete(TableMetadata).where(TableMetadata.id.in_(dup_ids))
                        )

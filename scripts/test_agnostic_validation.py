...
async def main():
    ...
    # A pergunta 5 requer encontrar a coluna de status sem usar _clean explicitamente hardcoded.
    ...
    if status1 == 200 and data1.get("answer") and not data1["meta"].get("error"):
        ...
        if "status_clean" in sql or "status_pt" in sql or "clean" in sql:
            print(f"   {GREEN}✅ PASSOU: SQL gerado corretamente usando convenção agnóstica.{RESET}")
    ...
    try:
        ...
        async with SessionLocal() as db:
            result = await db.execute(
                text("SELECT * FROM security_alerts WHERE user_id = :uid ORDER BY timestamp DESC LIMIT 1"),
                {"uid": USER_ID}
            )
            row = result.fetchone()
            ...
            if row:
                ...
                print(f"   Detalhes: {row.details}")
                 ...
                 print(f"   {RED}❌ FALHOU: Nenhum alerta encontrado para o usuário {USER_ID}.{RESET}")

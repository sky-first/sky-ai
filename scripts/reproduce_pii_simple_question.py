...
def test_scenario(name, question, sql, data):
    ...
    print(
        f"Text Scan: Detected={text_result.detected}, Types={text_result.pii_types}, Block={text_result.should_block}"
    )
    ...
    if data:
        ...
        print(
            f"Data Scan: Detected={data_result.detected}, Types={data_result.pii_types}, Block={data_result.should_block} (Types: {data_result.pii_types})"
        )
    ...
    if text_result.should_block:
        allow_text = check_func(
            question=question, sql=sql, data=data, pii_detection_result=text_result
        )
        print(f"Allow Text ({func_name})? {allow_text}")

    if data_result and data_result.should_block:
        allow_data = check_func(
            question=question, sql=sql, data=data, pii_detection_result=data_result
        )
...
test_scenario(
    ...
    [{"name": "Alice", "spend": 1000}, {"name": "Bob", "spend": 900}],
)

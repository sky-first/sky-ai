    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if (
                    "erro técnico" in answer.lower()
                    or "technical error" in answer.lower()
                ):
                    print("✅ Correct friendly error message received!")
                else:
                    print(
                        f"ℹ️ Received normal response (or other error): {answer[:100]}..."
                    )
            else:
                    ...
                    if line.startswith("data: "):
                        try:
                            ...
                            if data.get("type") == "error":
                                ...
                                if (
                                    "technical error" in msg.lower()
                                    or "erro técnico" in msg.lower()
                                ):
                                    print(
                                        "✅ Correct friendly stream error message received!"
                                    )
                            elif data.get("type") == "chunk":
                                content = data.get("content", "")
                                print(f"Chunk: {content[:50]}...")
                                if (
                                    "technical error" in content.lower()
                                    or "erro técnico" in content.lower()
                                ):
                                    print(
                                        "✅ Correct friendly chunk error message received!"
                                    )
                        except:
                            pass

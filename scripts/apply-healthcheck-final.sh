...
az vm run-command invoke \
    ...
    --scripts \
        ...
        'if grep -A 10 "frontend:" docker-compose.yml | grep -q "healthcheck:"; then echo "✅ Healthcheck já existe"; exit 0; fi' \
        ...
        'if grep -A 10 "frontend:" docker-compose.yml | grep -q "healthcheck:"; then echo "✅ Healthcheck aplicado"; else echo "❌ Erro"; exit 1; fi' \
        'docker compose down' \
        'docker compose up -d' \
        'sleep 20' \

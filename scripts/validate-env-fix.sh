...
echo "1️⃣ Verificando step 'Bootstrap .env from env.example'..."
if grep -q "Bootstrap .env from env.example (CRITICAL - Always Run)" .github/workflows/deploy.yml; then
    echo -e "${GREEN}✅ Step 'Bootstrap .env' encontrado${NC}"
else
    echo -e "${RED}❌ Step 'Bootstrap .env' NÃO encontrado${NC}"
    ((ERRORS++))
fi

# 2. Verificar se o step está na ordem correta (depois de Update Code)
echo ""
echo "2️⃣ Verificando ordem dos steps..."
...
if [ -n "$BOOTSTRAP_LINE" ] && [ -n "$UPDATE_CODE_LINE" ]; then
    if [ "$BOOTSTRAP_LINE" -gt "$UPDATE_CODE_LINE" ]; then
        echo -e "${GREEN}✅ Step 'Bootstrap .env' está DEPOIS de 'Update Code' (correto)${NC}"
    else
        echo -e "${RED}❌ Step 'Bootstrap .env' está ANTES de 'Update Code' (incorreto)${NC}"
        ((ERRORS++))
    fi
else
    echo -e "${YELLOW}⚠️  Não foi possível verificar ordem dos steps${NC}"
fi

# 3. Verificar se o step "Restart Containers" tem validações
echo ""
echo "3️⃣ Verificando validações no step 'Restart Containers'..."
...
if echo "$RESTART_SECTION" | grep -q "docker ps"; then
    echo -e "${GREEN}✅ Validação 'docker ps' encontrada${NC}"
else
    echo -e "${RED}❌ Validação 'docker ps' NÃO encontrada${NC}"
    ((ERRORS++))
fi

if echo "$RESTART_SECTION" | grep -q "localhost/health"; then
    echo -e "${GREEN}✅ Validação 'curl localhost/health' encontrada${NC}"
else
    echo -e "${RED}❌ Validação 'curl localhost/health' NÃO encontrada${NC}"
    ((ERRORS++))
fi

if echo "$RESTART_SECTION" | grep -q "RUNNING_CONTAINERS"; then
    echo -e "${GREEN}✅ Validação de contagem de containers encontrada${NC}"
else
    echo -e "${YELLOW}⚠️  Validação de contagem de containers não encontrada${NC}"
fi

# 4. Verificar se env.example existe
echo ""
echo "4️⃣ Verificando se env.example existe..."
if [ -f env.example ]; then
    ...
    for var in "${CRITICAL_VARS[@]}"; do
        if grep -q "^${var}=" env.example; then
            ...
            echo -e "   ${RED}❌${NC} $var (faltando)"
            ((ERRORS++))
        fi
    done
else
    echo -e "${RED}❌ env.example NÃO existe${NC}"
    ((ERRORS++))
fi

# 5. Verificar se smoke-tests.sh existe
echo ""
echo "5️⃣ Verificando se smoke-tests.sh existe..."
if [ -f scripts/smoke-tests.sh ]; then
    echo -e "${GREEN}✅ smoke-tests.sh existe${NC}"
    
    # Verificar se o script verifica .env
    if grep -q "\.env" scripts/smoke-tests.sh; then
        echo -e "   ${GREEN}✅${NC} Script verifica .env"
    else
        echo -e "   ${YELLOW}⚠️  ${NC} Script não verifica .env explicitamente"
    fi
else
    echo -e "${RED}❌ smoke-tests.sh NÃO existe${NC}"
    ((ERRORS++))
fi

# 6. Verificar sintaxe YAML do pipeline
echo ""
echo "6️⃣ Verificando estrutura do pipeline..."
# Verificação básica: arquivo existe e não está vazio
if [ -s .github/workflows/deploy.yml ]; then
    echo -e "${GREEN}✅ Arquivo deploy.yml existe e não está vazio${NC}"
    ...
    if grep -q "name:" .github/workflows/deploy.yml && grep -q "on:" .github/workflows/deploy.yml; then
        echo -e "${GREEN}✅ Estrutura básica de workflow encontrada${NC}"
    else
        echo -e "${RED}❌ Estrutura básica de workflow não encontrada${NC}"
        ((ERRORS++))
    fi
else
    echo -e "${RED}❌ Arquivo deploy.yml não existe ou está vazio${NC}"
    ((ERRORS++))
fi

# 7. Verificar documentação
echo ""
echo "7️⃣ Verificando documentação criada..."
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo "  ✅ Step 'Bootstrap .env' adicionado ao pipeline"
    echo "  ✅ Step está na ordem correta (após Update Code)"
    echo "  ✅ Validações adicionadas no step 'Restart Containers'"
    echo "  ✅ env.example existe e tem variáveis críticas"
    echo "  ✅ smoke-tests.sh existe"
    echo "  ✅ Pipeline YAML válido"

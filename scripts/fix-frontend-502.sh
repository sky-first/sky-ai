    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if echo "$result" | grep -q "Conflict"; then
            if [ $attempt -lt $max_attempts ]; then
                ...
                echo -e "${RED}❌ Timeout: Comando anterior ainda em execução após $max_attempts tentativas${NC}"
                return 1
            fi
        else
...
echo -e "${BLUE}2️⃣ Verificando logs antes do reinício...${NC}"
...
RESTART_RESULT=$(run_vm_command 'docker restart ai_saas_frontend_prod && echo "✅ Frontend reiniciado"')
echo "$RESTART_RESULT"
echo ""

echo -e "${BLUE}4️⃣ Aguardando frontend iniciar (20 segundos)...${NC}"
sleep 20
echo ""

...
if [ -f "$PROJECT_DIR/scripts/validate_structure.sh" ]; then
  ...
  echo -e "${YELLOW}[WARNING] Script de validação não encontrado, pulando...${NC}"
fi
...
echo -e "${GREEN}[OK] Docker está instalado e rodando${NC}"
...
echo -e "${GREEN}[OK] Docker Compose está instalado${NC}"
...
if docker compose -f "$PROJECT_DIR/docker-compose.yml" config &> /dev/null; then
  ...
  echo -e "${RED}[ERROR] Erro na validação do docker-compose.yml${NC}"
...
if [ ! -f "$PROJECT_DIR/.env" ]; then
  ...
  if [ -f "$PROJECT_DIR/env.example" ]; then
    ...
    echo -e "${YELLOW}[WARNING] IMPORTANTE: Edite .env e configure os valores:${NC}"
    ...
    echo -e "${RED}[ERROR] env.example não encontrado${NC}"
    exit 1
  fi
else
  ...
  if grep -q "secure_password_here\|generate_a_secure" "$PROJECT_DIR/.env"; then
    echo -e "${YELLOW}[WARNING] .env contém valores placeholder${NC}"
...
if [ "$AVAILABLE" -lt 10 ]; then
  echo -e "${YELLOW}[WARNING] Pouco espaço em disco: ${AVAILABLE}GB disponível${NC}"
  echo -e "${YELLOW}   Recomendado: pelo menos 10GB${NC}"
else
  echo -e "${GREEN}[OK] Espaço em disco suficiente: ${AVAILABLE}GB${NC}"
fi

# 7. Resumo
echo ""
echo -e "${GREEN}[OK] Preparação concluída!${NC}"
echo ""

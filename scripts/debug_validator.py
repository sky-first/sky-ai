...
def debug_validator():
    validator = AdvancedSQLValidator(allowed_tables=["project.dataset.table", "orders", "revenue"])
    
    # Guessing the query that failed
    print("--- Test 1: DATE_SUB in WHERE ---")
    sql1 = "SELECT * FROM orders WHERE date > DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH)"
    tables1 = validator._extract_tables(sql1) # Note: _extract_tables takes Statement, but in code it calls str(statement).

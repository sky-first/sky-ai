...
async def _update_job(
    job_id: str, updates: Dict[str, Any], db_session: AsyncSession = None
):
...
async def _append_log(
    job_id: str, message: str, level: str = "info", db_session: AsyncSession = None
):
    ...
    if db_session:
        result = await db_session.execute(
            select(PipelineJob.logs).where(PipelineJob.id == job_id)
        )
    ...
    async def _execute_pipeline_task():
        ...
        async with AsyncSessionLocal() as bg_db:
            try:
                ...
                await _update_job(
                    pipeline_id, {"status": "completed", "result": result_data}, bg_db
                )

                await _append_log(
                    pipeline_id, "Pipeline concluído com sucesso", "info", bg_db
                )

            except Exception as e:
                error_msg = str(e)
                await _update_job(
                    pipeline_id, {"status": "failed", "error": error_msg}, bg_db
                )

                await _append_log(
                    pipeline_id, f"Erro no pipeline: {error_msg}", "error", bg_db
                )

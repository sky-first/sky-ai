...
async def update_metadata(conn):
    ...
    print(f"  ✅ Semantic metadata inserted: {table_count} tables, {total_cols} columns")


def _extract_tags(table_name: str, col_name: str) -> list:
    """Return business-term tags for easier filtering/debugging."""
    tags_map = {
        ("sky_test_orders", "total_amount"):    ["revenue", "sales", "income", "earnings", "aov"],
        ("sky_test_orders", "discount_amount"): ["discount", "net_revenue", "promotion"],
        ("sky_test_orders", "status"):          ["completed", "cancelled", "refunded", "pending"],
        ("sky_test_orders", "region"):          ["region", "geography", "location"],
        ("sky_test_orders", "channel"):         ["channel", "online", "in-store"],
        ("sky_test_order_items", "subtotal"):   ["product_revenue", "line_revenue", "category_revenue"],
        ...
        ("sky_test_customers", "created_at"):   ["acquisition", "new_customers", "signup"],
        ("sky_test_customers", "last_order_at"):["churn", "inactive", "retention"],
        ("sky_test_customers", "segment"):      ["segment", "enterprise", "smb"],
        ("sky_test_sales_reps", "name"):        ["sales_rep", "salesperson", "rep_performance"],
    }

...
class TestComplexCases:
    ...
    def test_typos_and_variations(self):
        ...
        assert self.detector.detect("dashbord for sales") == False  # typo not caught (acceptable)
...
class TestDebugHelper:
    ...
    def test_get_matched_patterns(self):
        ...
        positive, negative = self.detector.get_matched_patterns("what's on my dashboard")
        assert len(positive) == 0
        assert len(negative) > 0

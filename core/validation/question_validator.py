...
class QuestionValidator:
    ...
    def __init__(
        self, available_tables: List[Dict], available_columns: Dict[str, List[str]]
    ):
        """
        Args:
    ...
    def _check_missing_references(self, question: str) -> List[str]:
        ...
        for ref in quoted_refs:
            ...
            if ref_clean:
                ...
                if (
                    ref_clean not in available_table_names
                    and ref_clean not in available_table_variations
                ):
                    # Verificar se não é uma substring de alguma tabela
                    is_substring = any(
                        ref_clean in name or name in ref_clean
                        for name in list(available_table_names)
                        + list(available_table_variations)
                    )
                    if not is_substring:
                        missing.append(ref)
        ...
        for pattern in table_ref_patterns:
            ...
            for match in matches:
                ...
                if match_clean:
                    if (
                        match_clean not in available_table_names
                        and match_clean not in available_table_variations
                    ):

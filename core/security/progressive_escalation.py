...
SCHEMA_EXPLORATION_KEYWORDS = [
    "quais tabelas", "what tables", "list tables",
    "quais colunas", "what columns", "list columns",
    "estrutura", "structure", "schema",
    "metadados", "metadata", "describe",
    "show tables", "show columns",
    "quais campos", "what fields",
    ...
    if REDIS_AVAILABLE and redis_client is not None:
        try:
            ...
            if schema_count >= max_schema_questions:
                return True, score, f"Multiple schema exploration questions ({schema_count} in {window_minutes} minutes)"

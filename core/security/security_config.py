...
class TableSecurityConfig(BaseModel):
    ...
    blocked_columns: Optional[List[str]] = Field(
        default=None, description="Lista de colunas bloqueadas."
    )
...
class SecurityConfig(BaseModel):
    ...
    global_blocked_columns: List[str] = Field(
        default_factory=list, description="Colunas bloqueadas em todas as tabelas."
    )
    max_rows_limit: int = Field(
        default=5000, description="Limite máximo de linhas por query."
    )
    allow_joins: bool = Field(
        default=True, description="Se permite JOINs entre tabelas."
    )
    allow_subqueries: bool = Field(
        default=False, description="Se permite subqueries (mais restritivo por padrão)."
    )
...
def filter_columns_by_security(
    columns: List[Any], table_name: str, security_config: SecurityConfig
) -> List[Any]:
    """
...
def get_row_filter_for_table(
    table_name: str, security_config: SecurityConfig
) -> Optional[str]:
    """
            ...
            if match:
                ...
                if "." not in row_filter:
                    ...
                    if col_match:
                        ...
                        row_filter = row_filter.replace(
                            col_name, f"{alias}.{col_name}", 1
                        )
...
def validate_sql_against_security(
    ...
    if limit_match:
        ...
        if limit_value > security_config.max_rows_limit:
            return (
                False,
                f"LIMIT {limit_value} exceeds maximum allowed ({security_config.max_rows_limit})",
            )
    else:
    ...
    for table_name, config in security_config.tables.items():
        ...
        if re.search(rf"\b{re.escape(table_name)}\b", sql, re.IGNORECASE):
            ...
            if filter_col_match:
                ...
                if not re.search(
                    rf"\bWHERE\b.*\b{re.escape(filter_col)}\b", sql, re.IGNORECASE
                ):
                    return (
                        False,
                        f"Required row filter for table '{table_name}' is missing",
                    )
...
def build_security_prompt_instructions(security_config: SecurityConfig) -> str:
    ...
    for table_name, config in security_config.tables.items():
        if config.row_filter:
            rls_rules.append(
                f"   - {table_name}: MUST include WHERE {config.row_filter}"
            )

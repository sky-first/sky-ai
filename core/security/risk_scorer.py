...
SUSPICIOUS_PATTERNS = [
    ...
    (
        re.compile(
            r"\bignore\s+(?:all|todas|toutes)\s+(?:rules?|regras|règles)\b",
            re.IGNORECASE,
        ),
        4,
    ),
    ...
    (
        re.compile(
            r"\bignore\s+previous\s+(?:instructions?|instruções)\b", re.IGNORECASE
        ),
        4,
    ),
    ...
    (
        re.compile(r"\b(show|list|quais|which)\s+(?:all\s+)?columns?\b", re.IGNORECASE),
        2,
    ),
    (
        re.compile(r"\bshow\s+me\s+all\s+tables?\b", re.IGNORECASE),
        2,
    ),  # Específico para "show me all tables"
    ...
    (
        re.compile(
            r'\{\s*"\$(ne|gt|lt|where|regex|expr|jsonSchema|function|accumulator)"\s*:',
            re.IGNORECASE,
        ),
        4,
    ),
    (
        re.compile(
            r"\bdb\.[a-zA-Z0-9_]+\.(find|update|remove|drop|aggregate|mapReduce)\(",
            re.IGNORECASE,
        ),
        4,
    ),
    ...
    (
        re.compile(r'(\|\s*\||&&)\s*["\']\w+["\']\s*==\s*["\']\w+["\']', re.IGNORECASE),
        3,
    ),
]

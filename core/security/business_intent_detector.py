...
BUSINESS_INTENT_PATTERNS = [
    ...
    (r'\b(mais|most|menos|least)\s+(utilizado|used|frequente|frequent|comum|common)', re.IGNORECASE),
    
    # "qual é o maior/menor", "which is the highest/lowest"
    (r'\b(qual|which|quelles|welche|quali)\s+(é|is|are|são)\s+(o|a|o|os|as|the)\s+(maior|menor|mais|menos|highest|lowest)', re.IGNORECASE),
    
    # "top N", "ranking", "classificação"
    (r'\b(top|ranking|rank|classificação|classificación|classement)\s+\d*', re.IGNORECASE),
    (r'\b(melhores|best|piores|worst)\s+\d*', re.IGNORECASE),
    
    # ========== PADRÕES DE MÉTRICAS E TOTAIS ==========
    # "total", "soma", "média", "contagem"
    (r'\b(total|soma|sum|média|average|mean|contagem|count|quantidade|quantity)\s+(de|of|por|per)', re.IGNORECASE),
    ...
    (r'\b(distribuição|distribution|distribución|répartition|verteilung)\s+(?:de|of|por|by)', re.IGNORECASE),
    ...
    (r'\b(evolução|evolution|evolución|évolution|tendência|trend|tendencia)', re.IGNORECASE),
    ...
    (r'\b(análise|analysis|analyse|analisi|performance|desempenho|rendimiento)', re.IGNORECASE),
    (r'\b(métrica|metric|métrique|indicador|indicator|indicatore)\s+(?:de|of|por|by)', re.IGNORECASE),
    
    # ========== PADRÕES DE COMPARAÇÃO ==========
    # "quanto", "how much", "qual a diferença"
    (r'\b(quanto|how\s+much|qual\s+a\s+diferença|what\s+is\s+the\s+difference)', re.IGNORECASE),
    
    # ========== PADRÕES DE PERCENTUAL/PROPORÇÃO ==========
    # "percentual", "porcentagem", "proporção"
    (r'\b(percentual|percentage|porcentaje|proporção|proportion)\s+(?:de|of|por|by)', re.IGNORECASE),
    
    # ========== PADRÕES DE MÉTODO/TIPO/FORMA ==========
    # "qual método", "which method", "qual tipo", "what type"
    (r'\b(qual|which|what)\s+(método|method|tipo|type|forma|way|modalidade|modality)', re.IGNORECASE),
    
    # "mais utilizado", "most used" (reforço)
    (r'\b(método|method|tipo|type|forma|way)\s+(?:é|is)\s+(?:mais|most)\s+(?:utilizado|used)', re.IGNORECASE),
]

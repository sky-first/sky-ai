...
class UserContext:
    ...
    )  # Supporting multiple spaces for "Personal" mode
...
class QueryContext:
    ...
    entities: List[str] = field(
        default_factory=list
    )  # Detected entities (customers, products, etc)
    metrics: List[str] = field(
        default_factory=list
    )  # Detected metrics (revenue, count, etc)
...
class HistoricalContext:
    ...
    catalog_rag: List[str] = field(
        default_factory=list
    )  # Data Catalog, Conexões, Infraestrutura
    analytics_rag: List[str] = field(
        default_factory=list
    )  # Lineage, Qualidade, Compliance de dados
    strategy_rag: List[str] = field(
        default_factory=list
    )  # Objetivos, OKRs, Metas corporativas
    governance_rag: List[str] = field(
        default_factory=list
    )  # Governança, Permissões, Auditoria
    enterprise_rag: List[str] = field(
        default_factory=list
    )  # Grafos da empresa, Relações sistêmicas
    signals_rag: List[str] = field(
        default_factory=list
    )  # Sazonalidade, Eventos, Ciclos macroeconômicos

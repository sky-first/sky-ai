...
def _serialize_for_orchestrator(bundle: ContextBundle) -> str:
    ...
    if bundle.data.relationships and len(bundle.data.tables) > 1:
        explicit_rels = [
            r
            for r in bundle.data.relationships
            if getattr(r, "confidence", "inferred") == "explicit"
        ]
        inferred_rels = [
            r
            for r in bundle.data.relationships
...
def _serialize_for_specialist(bundle: ContextBundle) -> str:
    ...
    if bundle.historical.governance_rag:
        ...
        for (
            ...
            bundle.historical.governance_rag
        ):  # Injeta TODAS as políticas de gov retidas
            parts.append(f"- {chunk}")
...
def _serialize_full(bundle: ContextBundle) -> str:
    ...
    # User context
    parts.append(
        f"[USER: {bundle.user.user_id} | Role: {bundle.user.platform_role}/{bundle.user.crew_role}]"
    )
    ...
    parts.append(
        f"- Entities: {', '.join(bundle.query.entities) if bundle.query.entities else 'none'}"
    )
    parts.append(
        f"- Metrics: {', '.join(bundle.query.metrics) if bundle.query.metrics else 'none'}"
    )

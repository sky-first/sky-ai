...
class DavinciPlanValidator:
    ...
    def validate_plan(
        self, plan: Dict[str, Any], context_provided: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def _check_temporal_agreement(self, plan: Dict[str, Any]) -> List[str]:
            ...
            for c in t.get("columns", []):
                try:
                    ...
                    if c_min:
                        ...
                        if isinstance(c_min, str):
                            c_min = datetime.fromisoformat(c_min.split(" ")[0])
                        if not min_date_found or c_min < min_date_found:
                            min_date_found = c_min
                    if c_max:
                        if isinstance(c_max, str):
                            c_max = datetime.fromisoformat(c_max.split(" ")[0])
                        if not max_date_found or c_max > max_date_found:
                            max_date_found = c_max
                except:
                    continue
        ...
        if (
            "yearly" in narrative
            or "last year" in narrative
            or "year-over-year" in narrative
        ):
            if days_covered < 365:
                issues.append(
                    f"Temporal Hallucination: Plan mentions 'yearly' trends, but data only covers {days_covered} days."
                )
        ...
        if "months" in narrative or "monthly" in narrative:
            ...
            if match:
                ...
                if days_covered < (months_claimed * 28):
                    issues.append(
                        f"Temporal Hallucination: Claimed '{months_claimed} months' of history, but data only covers {days_covered} days."
                    )
            elif days_covered < 20:
                # General mentions of 'monthly' should have at least some weeks of data
                issues.append(
                    f"Temporal Hallucination: Mentions 'monthly' performance, but data only covers {days_covered} days."
                )
            ...
            for key in ["x_axis_column", "y_axis_column", "date_column"]:
                ...
                if col_ref:
                    ...
                    if not self._column_exists_anywhere(col_ref):
                        issues.append(
                            f"Widget '{title}': Column '{col_ref}' not found in any logical table."
                        )

        return issues

    def _check_ungrounded_claims(
        self, plan: Dict[str, Any], context_provided: bool
    ) -> List[str]:
        """
        ...
        for section in sections:
            ...
            for pattern in risk_patterns:
                if re.search(pattern, text, re.IGNORECASE):
                    issues.append(
                        f"Potential Hallucination within '{section}': Found specific claim '{pattern}' without prior data context."
                    )

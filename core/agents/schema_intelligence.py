...
class SchemaScorer:
    ...
    @classmethod
    def extract_features(cls, table_name: str, columns: List[Dict[str, Any]], row_count: int) -> TableFeatures:
        ...
        for col in columns:
            ...
            # Numeric (Measures)
            if any(t in ctype for t in ["INT", "FLOAT", "NUMERIC", "DECIMAL", "DOUBLE", "REAL"]):
        ...
        for col in columns:
            ...
            col_features_list.append(ColumnFeatures(
                name=cname,
                dtype=ctype,
                is_numeric=is_num,
                is_text=is_txt,
    ...
    def score_table(cls, features: TableFeatures) -> Dict[str, Any]:
        ...
        return {
            ...
            "scores": {
                "fact": round(fact_score, 2),
                "dimension": round(dim_score, 2)
            },
            "signals": {
...
class AnalyticTableFilter:
    ...
    SYSTEM_PREFIXES = {
        "django_", "auth_", "knex_", "alembic_", "audit_", "celery_", "pg_", "sql_", "ar_internal_"
    }
    
    SYSTEM_SUFFIXES = {
        "_migration", "_migrations", "_audit", "_log", "_backup", "_tmp", "_temp"
    }
    
    @classmethod
    def should_exclude(cls, table_name: str, row_count: int = 1, allow_logs: bool = False) -> bool:
...
class JoinScorer:
    ...
    @staticmethod
    def score_join(
        t1_name: str, c1_name: str, 
        t2_name: str, c2_name: str
    ) -> float:
        """

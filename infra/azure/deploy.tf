  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      $scripts = @(
        ...
        'CONTAINER_STATUS=$(sudo docker compose ps --format json 2>/dev/null || echo "[]")',
        'if echo "$CONTAINER_STATUS" | grep -q "ai_saas_proxy"; then',
        '  echo "✅ Proxy container iniciado"',
        'else',
        '  echo "❌ ERRO: Proxy container NÃO iniciou"',
        '  sudo docker compose ps',
        '  sudo docker compose logs proxy --tail=30 2>/dev/null || echo "Logs do proxy não disponíveis"',
        '  exit 1',
        '}',
